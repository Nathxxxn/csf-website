...
export const ContainerTextScroll = ({
  ...
  const translateY = useTransform(scrollYProgress, [0, 0.45], [100, 0]);
  const titleTranslateY = useTransform(scrollYProgress, [0, 0.45], [100, 0]);
  const titleScale = useTransform(scrollYProgress, [0, 0.45], [0.8, 1]);

  return (
    <div
      ref={containerRef}
      className="h-220 md:h-280 flex items-center justify-center relative p-2 md:p-20"
    >
      <motion.div
        style={{ translateY }}
...
export const Card = ({
  ...
  return (
    <motion.div
      ...
      className="relative h-152 md:h-160 w-full bg-[#050505] rounded-[30px] shadow-2xl overflow-hidden"
    >
